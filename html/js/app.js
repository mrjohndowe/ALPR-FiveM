...
$('#alpr-markers-form').submit(function onSubmit(e) {
  ...
  if (markerPayload.licensePlate === '') return;
  if (markerPayload.firstName === '') return;
  if (markerPayload.lastName === '') return;
  ...
  if (isDeletion) {
    ...
    // ✅ FIX: use findIndex instead of find
    const deletionIndex = markers.findIndex(m => m.markerId === currentMarkerId);

    if (deletionIndex > -1) {
      const markerToDelete = markers[deletionIndex];

      // remove from array properly
      markers.splice(deletionIndex, 1);

      // remove button from UI
      $(`[marker-id="${markerToDelete.markerId}"]`).remove();
    }

    $.post(
      'https://alpr/deleteMarker',
      JSON.stringify({
        markerId: currentMarkerId,
        plate: markerPayload.licensePlate
      })
    );
